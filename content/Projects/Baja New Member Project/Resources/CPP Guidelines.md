...
When commenting, please note [[#In: Introduction|the introduction]] that outlines our aims and general approach.
The list of contributors is [[#Acknowledgements|here]].
...
You can [[#Abstract|read an explanation of the scope and structure of this Guide]] or just jump straight in:
...
* assignment:
[[#C.11: Make concrete types regular|regular types]] --
[[#C.49: Prefer initialization to assignment in constructors|prefer initialization]] --
[[#C.61: A copy operation should copy|copy]] --
[[#C.64: A move operation should move and leave its source in a valid state|move]] --
[[#C.22: Make default operations consistent|other operations]] --
[[#C.80: Use `=default` if you have to be explicit about using the default semantics|default]]
* `class`:
[[#C.1: Organize related data into structures (`struct`s or `class`es)|data]] --
[[#C.2: Use `class` if the class has an invariant; use `struct` if the data members can vary independently|invariant]] --
[[#C.4: Make a function a member only if it needs direct access to the representation of a class|members]] --
[[#C.5: Place helper functions in the same namespace as the class they support|helpers]] --
[[#C.concrete: Concrete types|concrete types]] --
[[#C.ctor: Constructors, assignments, and destructors|ctors, =, and dtors]] --
[[#C.hier: Class hierarchies (OOP)|hierarchy]] --
[[#C.over: Overloading and overloaded operators|operators]]
* `concept`:
[[#T.concepts: Concept rules|rules]] --
[[#T.1: Use templates to raise the level of abstraction of code|in generic programming]] --
[[#T.10: Specify concepts for all template arguments|template arguments]] --
[[#T.20: Avoid "concepts" without meaningful semantics|semantics]]
* constructor:
[[#C.2: Use `class` if the class has an invariant; use `struct` if the data members can vary independently|invariant]] --
[[#C.40: Define a constructor if a class has an invariant|establish invariant]] --
[[#C.42: If a constructor cannot construct a valid object, throw an exception|`throw`]] --
[[#C.43: Ensure that a copyable class has a default constructor|default]] --
[[#C.45: Don't define a default constructor that only initializes data members; use default member initializers instead|not needed]] --
[[#C.46: By default, declare single-argument constructors explicit|`explicit`]] --
[[#C.51: Use delegating constructors to represent common actions for all constructors of a class|delegating]] --
[[#C.82: Don't call virtual functions in constructors and destructors|`virtual`]]
* derived `class`:
[[#C.120: Use class hierarchies to represent concepts with inherent hierarchical structure (only)|when to use]] --
[[#C.121: If a base class is used as an interface, make it a pure abstract class|as interface]] --
[[#C.127: A class with a virtual function should have a virtual or protected destructor|destructors]] --
[[#C.130: For making deep copies of polymorphic classes prefer a virtual `clone` function instead of public copy construction/assignment|copy]] --
[[#C.131: Avoid trivial getters and setters|getters and setters]] --
[[#C.135: Use multiple inheritance to represent multiple distinct interfaces|multiple inheritance]] --
[[#C.138: Create an overload set for a derived class and its bases with `using`|overloading]] --
[[#C.67: A polymorphic class should suppress public copy/move|slicing]] --
[[#C.146: Use `dynamic_cast` where class hierarchy navigation is unavoidable|`dynamic_cast`]]
* destructor:
[[#C.22: Make default operations consistent|and constructors]] --
[[#C.30: Define a destructor if a class needs an explicit action at object destruction|when needed?]] --
[[#C.36: A destructor must not fail|must not fail]]
* exception:
[[#E: Error handling|errors]] --
[[#E.2: Throw an exception to signal that a function can't perform its assigned task|`throw`]] --
[[#E.3: Use exceptions for error handling only|for errors only]] --
[[#E.12: Use `noexcept` when exiting a function because of a `throw` is impossible or unacceptable|`noexcept`]] --
[[#E.18: Minimize the use of explicit `try`/`catch`|minimize `try`]] --
[[#E.27: If you can't throw exceptions, use error codes systematically|what if no exceptions?]]
* `for`:
[[#ES.71: Prefer a range-`for`-statement to a `for`-statement when there is a choice|range-for and for]] --
[[#ES.72: Prefer a `for`-statement to a `while`-statement when there is an obvious loop variable|for and while]] --
[[#ES.74: Prefer to declare a loop variable in the initializer part of a `for`-statement|for-initializer]] --
[[#ES.85: Make empty statements visible|empty body]] --
[[#ES.86: Avoid modifying loop control variables inside the body of raw for-loops|loop variable]] --
[[#loop variable type ???]]
* function:
[[#F.1: "Package" meaningful operations as carefully named functions|naming]] --
[[#F.2: A function should perform a single logical operation|single operation]] --
[[#F.6: If your function must not throw, declare it `noexcept`|no throw]] --
[[#F.7: For general use, take `T*` or `T&` arguments rather than smart pointers|arguments]] --
[[#F.15: Prefer simple and conventional ways of passing information|argument passing]] --
[[#F.21: To return multiple "out" values, prefer returning a struct|multiple return values]] --
[[#F.42: Return a `T*` to indicate a position (only)|pointers]] --
[[#F.50: Use a lambda when a function won't do (to capture local variables, or to write a local function)|lambdas]]
* `inline`:
[[#F.5: If a function is very small and time-critical, declare it `inline`|small functions]] --
[[#SF.2: A header file must not contain object definitions or non-inline function definitions|in headers]]
* initialization:
[[#ES.20: Always initialize an object|always]] --
[[#ES.23: Prefer the `{}`-initializer syntax|prefer `{}`]] --
[[#ES.28: Use lambdas for complex initialization, especially of `const` variables|lambdas]] --
[[#C.48: Prefer default member initializers to member initializers in constructors for constant initializers|default member initializers]] --
[[#C.49: Prefer initialization to assignment in constructors|class members]] --
[[#C.50: Use a factory function if you need "virtual behavior" during initialization|factory functions]]
* lambda expression:
[[#C.lambdas: Function objects and lambdas|when to use]]
* operator:
[[#C.160: Define operators primarily to mimic conventional usage|conventional]] --
[[#C.164: Avoid implicit conversion operators|avoid conversion operators]] --
[[#C.170: If you feel like overloading a lambda, use a generic lambda|and lambdas]]
* `public`, `private`, and `protected`:
[[#C.9: Minimize exposure of members|information hiding]] --
[[#C.134: Ensure all non-`const` data members have the same access level|consistency]] --
[[#C.133: Avoid `protected` data|`protected`]]
* `static_assert`:
[[#P.5: Prefer compile-time checking to run-time checking|compile-time checking]] --
[[#T.150: Check that a class matches a concept using `static_assert`|and concepts]]
* `struct`:
[[#C.1: Organize related data into structures (`struct`s or `class`es)|for organizing data]] --
[[#C.2: Use `class` if the class has an invariant; use `struct` if the data members can vary independently|use if no invariant]] --
[[#C.8: Use `class` rather than `struct` if any member is non-public|no private members]]
* `template`:
[[#T.1: Use templates to raise the level of abstraction of code|abstraction]] --
[[#T.3: Use templates to express containers and ranges|containers]] --
[[#T.10: Specify concepts for all template arguments|concepts]]
* `unsigned`:
[[#ES.100: Don't mix signed and unsigned arithmetic|and signed]] --
[[#ES.101: Use unsigned types for bit manipulation|bit manipulation]]
* `virtual`:
[[#I.25: Prefer empty abstract classes as interfaces to class hierarchies|interfaces]] --
[[#C.10: Prefer concrete types over class hierarchies|not `virtual`]] --
[[#C.35: A base class destructor should be either public and virtual, or protected and non-virtual|destructor]] --
[[#C.36: A destructor must not fail|never fail]]
...
## In.target: Target readership
All C++ programmers. This includes [[#CPL: C-style programming|programmers who might consider C]].
...
The rules are designed to allow [[#Appendix B: Modernizing code|gradual adoption]].
...
If you need a tutorial for some given level of experience, see [[#RF: References|the references]].
...
However, see [[#Appendix B: Modernizing code|the modernization section]] for some possible approaches to modernizing/rejuvenating/upgrading.
...
* `"tag"` is a string literal with the anchor name of the item where the Enforcement rule appears (e.g., for [[#C.134: Ensure all non-`const` data members have the same access level|C.134]] it is "Rh-public"), the
name of a profile group-of-rules ("type", "bounds", or "lifetime"),
or a specific rule in a profile ([[#type.4]], or [[#bounds.2]]). Any text that is not one of those should be rejected.
...
Such information is found in the **Alternative** paragraphs and the [[#Appendix C: Discussion|Discussion]] sections.
...
Recommended information sources can be found in [[#RF: References|the references]].
...
Avoid dependence on undefined behavior (e.g., [[#ES.43: Avoid expressions with undefined order of evaluation|undefined order of evaluation]])
...
For more details about for-statements, see [[#ES.71: Prefer a range-`for`-statement to a `for`-statement when there is a choice|ES.71]].
...
Prefer [[#R.1: Manage resources automatically using resource handles and RAII (Resource Acquisition Is Initialization)|RAII]]:
...
**See also**: [[#R: Resource management|The resource management section]]
...
Enforcing [[#Pro.lifetime: Lifetime safety profile|the lifetime safety profile]] eliminates leaks.
When combined with resource safety provided by [[#R.1: Manage resources automatically using resource handles and RAII (Resource Acquisition Is Initialization)|RAII]], it eliminates the need for "garbage collection" (by generating no garbage).
Combine this with enforcement of [[#In.force: Enforcement|the type and bounds profiles]] and you get complete type- and resource-safety, guaranteed by tools.
...
This is a variant of the [[#In.0: Don't panic!|subset of superset principle]] that underlies these guidelines.
...
* [[#CP.9: Whenever feasible use tools to validate your concurrent code|Concurrency tools]]
...
* [[#C.hier: Class hierarchies (OOP)|C.hier: Class hierarchies]]
...
**References**: See the [[#F.call: Parameter passing|rules for calling functions]].
...
No, using `unsigned` is not a good way to sidestep the problem of [[#ES.106: Don't try to avoid negative values by using `unsigned`|ensuring that a value is non-negative]].
...
Better still, use [[#R.1: Manage resources automatically using resource handles and RAII (Resource Acquisition Is Initialization)|RAII]] to ensure that the postcondition ("the lock must be released") is enforced in code:
...
**Alternative**: Postconditions of the form "this resource must be released" are best expressed by [[#R.1: Manage resources automatically using resource handles and RAII (Resource Acquisition Is Initialization)|RAII]].
...
**See also**: [[#T.gp: Generic programming|Generic programming]] and [[#T.concepts: Concept rules|concepts]].
...
* A good rule for performance critical code is to move checking outside the [[#Per.3: Don't optimize something that's not performance critical|critical]] part of the code.
* In the longer term, more regular code gets better optimized.
* Always carefully [[#Per.6: Don't make claims about performance without measurements|measure]] before making performance claims.

**See also**: [[#I.5: State preconditions (if any)|I.5]] and [[#I.7: State postconditions|I.7]] for reporting precondition and postcondition violations.
...
**Alternative**: [[#R.30: Take smart pointers as parameters only to explicitly express lifetime semantics|Pass ownership]] using a "smart pointer", such as `unique_ptr` (for exclusive ownership) and `shared_ptr` (for shared ownership).
...
**See also**: [[#F.15: Prefer simple and conventional ways of passing information|Argument passing]], [[#R.30: Take smart pointers as parameters only to explicitly express lifetime semantics|use of smart pointer arguments]], and [[#value return]].
...
This violated the rule [[#ES.20: Always initialize an object|against uninitialized variables]],
the rule against [[#I.11: Never transfer ownership by a raw pointer (`T*`) or reference (`T&`)|ignoring ownership]],
and the rule [[#ES.45: Avoid "magic constants"; use symbolic constants|against magic constants]].
...
* [[#F.5: If a function is very small and time-critical, declare it `inline`|F.5: If a function is very small and time-critical, declare it inline]]
...
* [[#F.48: Don't `return std::move(local)`|F.48: Don't return `std::move(local)`]]
...
The rule [[#F.3: Keep functions short and simple|Keep functions short and simple]] implies "Keep loop bodies short."
Similarly, lambdas used as callback arguments are sometimes non-trivial, yet unlikely to be reusable.

##### Enforcement

* See [[#F.3: Keep functions short and simple|Keep functions short and simple]]
...
See also [[#C.44: Prefer default constructors to be simple and non-throwing|C.44]].
...
We can catch many common cases of dangling pointers statically (see [[#Pro.lifetime: Lifetime safety profile|lifetime safety profile]]). Function arguments naturally live for the lifetime of the function call, and so have fewer lifetime problems.
...
* [[#F.60: Prefer `T*` over `T&` when "no argument" is a valid option|Prefer `T*` over `T&` when "no argument" is a valid option]]
...
For passing sequences of characters see [[#SL.str: String|String]].

##### Exception

To express shared ownership using `shared_ptr` types, rather than following guidelines F.16-21,
follow [[#R.34: Take a `shared_ptr<widget>` parameter to express shared ownership|R.34]], [[#R.35: Take a `shared_ptr<widget>&` parameter to express that a function might reseat the shared pointer|R.35]], and [[#R.36: Take a `const shared_ptr<widget>&` parameter to express that it might retain a reference count to the object ???|R.36]].
...
* If the function is going to unconditionally move from the argument, take it by `&&`. See [[#F.18: For "will-move-from" parameters, pass by `X&&` and `std::move` the parameter|F.18]].
...
* If the function is going to keep a copy of the argument to pass to another destination (to another function, or store in a non-local location), in addition to passing by `const&` (for lvalues),
  add an overload that passes the parameter by `&&` (for rvalues) and in the body `std::move`s it to its destination. Essentially this overloads a "will-move-from"; see [[#F.18: For "will-move-from" parameters, pass by `X&&` and `std::move` the parameter|F.18]].
* In special cases, such as multiple "input + copy" parameters, consider using perfect forwarding. See [[#F.19: For "forward" parameters, pass by `TP&&` and only `std::forward` the parameter|F.19]].
...
Most rumors about performance advantages from passing by `&&` are false or brittle (but see [[#F.18: For "will-move-from" parameters, pass by `X&&` and `std::move` the parameter|F.18]] and [[#F.19: For "forward" parameters, pass by `TP&&` and only `std::forward` the parameter|F.19]]).
...
To express shared ownership using `shared_ptr` types, follow [[#R.34: Take a `shared_ptr<widget>` parameter to express shared ownership|R.34]] or [[#R.36: Take a `const shared_ptr<widget>&` parameter to express that it might retain a reference count to the object ???|R.36]],
...
are [[#F.16: For "in" parameters, pass cheaply-copied types by value and others by reference to `const`|cheap to copy]] and may be passed by value, while doing so has
...
[[#C.64: A move operation should move and leave its source in a valid state|That could be dangerous]].
...
If the "will-move-from" parameter is a `shared_ptr` follow [[#R.34: Take a `shared_ptr<widget>` parameter to express shared ownership|R.34]] and pass the `shared_ptr` by value.
...
If you have multiple values to return, [[#F.21: To return multiple "out" values, prefer returning a struct|use a tuple]] or similar multi-member type.
...
* To reuse an object that carries capacity (e.g., `std::string`, `std::vector`) across multiple calls to the function in an inner loop: [[#F.21: To return multiple "out" values, prefer returning a struct|treat it as an in/out parameter and pass by reference]].
...
In such cases, passing the object by reference [[#F.17: For "in-out" parameters, pass by reference to non-`const`|`T&`]] is usually the right technique.
...
Note this is different from the `return move(...)` anti-pattern from [[#ES.56: Write `std::move()` only when you need to explicitly move an object to another scope|ES.56]].
...
**See also**: [[#I.13: Do not pass an array as a single pointer|Do not pass an array as a single pointer]]
...
**See also**: [[#C.50: Use a factory function if you need "virtual behavior" during initialization|C.50]] regarding when to return a `shared_ptr` from a factory.
...
A reference is often a superior alternative to a pointer [[#F.60: Prefer `T*` over `T&` when "no argument" is a valid option|if there is no need to use `nullptr`]] or [[#Con: Constants and immutability|if the object referred to should not change]].

##### Note

Do not return a pointer to something that is not in the caller's scope; see [[#F.43: Never (directly or indirectly) return a pointer or a reference to a local object|F.43]].
...
The [[#Pro.lifetime: Lifetime safety profile|lifetime safety profile]] will (when completely implemented) catch such problems.


##### Example

Returning an rvalue reference is fine when the reference to the temporary is being passed "downward" to a callee;
then, the temporary is guaranteed to outlive the function call (see [[#F.18: For "will-move-from" parameters, pass by `X&&` and `std::move` the parameter|F.18]] and [[#F.19: For "forward" parameters, pass by `TP&&` and only `std::forward` the parameter|F.19]]).
...
**See also**: [[#F.20: For "out" output values, prefer return values to output parameters|F.20, the general item about "out" output values]]
...
[[#C.140: Do not provide different default arguments for a virtual function and an overrider|Default arguments for virtual functions]]
...
* [[#C.8: Use `class` rather than `struct` if any member is non-public|define a class with private data as `class`]]
* [[#NL.16: Use a conventional class member declaration order|Prefer to place the interface first in a class]]
* [[#C.9: Minimize exposure of members|minimize exposure of members]]
* [[#C.133: Avoid `protected` data|Avoid `protected` data]]
...
This is especially important for [[#C.168: Define overloaded operators in the namespace of their operands|overloaded operators]].
...
Prefer to place the interface first in a class, [[#NL.16: Use a conventional class member declaration order|see NL.16]].
...
[[#C.133: Avoid `protected` data|`protected` data is a bad idea]].

##### Note

Prefer the order `public` members before `protected` members before `private` members; see [[#NL.16: Use a conventional class member declaration order|NL.16]].

##### Enforcement

* [[#C.133: Avoid `protected` data|Flag protected data]].
...
* [[#C.20: If you can avoid defining default operations, do|C.20: If you can avoid defining any default operations, do]]
...
* [[#C.45: Don't define a default constructor that only initializes data members; use default member initializers instead|C.45: Don't define a default constructor that only initializes data members; use member initializers instead]]
* [[#C.46: By default, declare single-argument constructors explicit|C.46: By default, declare single-argument constructors `explicit`]]
...
* [[#C.84: A `swap` function must not fail|C.84: A `swap` must not fail]]
* [[#C.85: Make `swap` `noexcept`]]
* [[#C.86: Make `==` symmetric with respect to operand types and `noexcept`|C.86: Make `==` symmetric with respect of operand types and `noexcept`]]
* [[#C.87: Beware of `==` on base classes]]
* [[#C.89: Make a `hash` `noexcept`]]
* [[#C.90: Rely on constructors and assignment operators, not `memset` and `memcpy`|C.90: Rely on constructors and assignment operators, not memset and memcpy]]
...
To prevent slicing as per [[#C.67: A polymorphic class should suppress public copy/move|C.67]],
...
To avoid the tedium and the possibility of errors, try to follow the [[#C.20: If you can avoid defining default operations, do|rule of zero]].
...
For most classes the answer is "no" either because the class holds no resources or because destruction is handled by [[#C.20: If you can avoid defining default operations, do|the rule of zero]];
that is, its members can take care of themselves as concerns destruction.
If the answer is "yes", much of the design of the class follows (see [[#C.21: If you define or `=delete` any copy, move, or destructor function, define or `=delete` them all|the rule of five]]).
...
What about a socket that won't close? A destructor, close, or cleanup operation [[#C.36: A destructor must not fail|should never fail]].
...
See [[#Discussion: Destructors, deallocation, and swap must never fail|discussion]].
...
Ownership should be clear in new code (and refactored legacy code) according to [[#R.20: Use `unique_ptr` or `shared_ptr` to represent ownership|R.20]] for owning
pointers and [[#R.3: A raw pointer (a `T*`) is non-owning|R.3]] for non-owning pointers.  References should never own [[#R.4: A raw reference (a `T&`) is non-owning|R.4]].
...
[[#R.3: A raw pointer (a `T*`) is non-owning|A `T*` should not do so]], but in older code, that's common.
...
Note that if you define a destructor, you must define or delete [[#C.21: If you define or `=delete` any copy, move, or destructor function, define or `=delete` them all|all default operations]]:
...
See [[#Discussion: Make base class destructors public and virtual, or protected and non-virtual|this in the Discussion section]].
...
##### Reason
 ...
 [[#C.36: A destructor must not fail|A destructor must not fail]]. If a destructor tries to exit with an exception, it's a bad design error and the program had better terminate.
...
Also, the default for `int` would be better done as a [[#C.48: Prefer default member initializers to member initializers in constructors for constant initializers|default member initializer]].

**See also**: [[#C.41: A constructor should create a fully initialized object|construct valid object]] and [[#C.42: If a constructor cannot construct a valid object, throw an exception|constructor throws]].
...
If a valid object cannot conveniently be constructed by a constructor, [[#C.50: Use a factory function if you need "virtual behavior" during initialization|use a factory function]].
...
If a constructor acquires a resource (to create a valid object), that resource should be [[#C.31: All resources acquired by a class must be released by the class's destructor|released by the destructor]].
The idiom of having constructors acquire resources and destructors release them is called [[#R.1: Manage resources automatically using resource handles and RAII (Resource Acquisition Is Initialization)|RAII]] ("Resource Acquisition Is Initialization").
...
There the `is_valid()` technique must be used. In such cases, check `is_valid()` consistently and immediately to simulate [[#R.1: Manage resources automatically using resource handles and RAII (Resource Acquisition Is Initialization)|RAII]].

##### Alternative

If you feel tempted to use some "post-constructor initialization" or "two-stage initialization" idiom, try not to do that.
If you really have to, look at [[#C.50: Use a factory function if you need "virtual behavior" during initialization|factory functions]].
...
[[#C.51: Use delegating constructors to represent common actions for all constructors of a class|Delegating constructors]] and [[#C.48: Prefer default member initializers to member initializers in constructors for constant initializers|default member initialization]] do that better.
Another reason has been to delay initialization until an object is needed; the solution to that is often [[#ES.22: Don't declare a variable until you have a value to initialize it with|not to declare a variable until it can be properly initialized]].
...
However, most realistic `Date` classes have a "first date" (e.g. January 1, 1970 is popular), so making that the default is usually trivial.
    ...
    public:
        Date(int dd, int mm, int yyyy);
        Date() = default; // [[#C.45: Don't define a default constructor that only initializes data members; use default member initializers instead|See also]]
        // ...
    private:
        int dd {1};
        int mm {1};
...
**See also**: [[#C.164: Avoid implicit conversion operators|Discussion of implicit conversions]]
...
**See also**: [[#Discussion: Define and initialize data members in the order of member declaration|Discussion]]
...
as [[#SL.str.2: Use `std::string_view` or `gsl::span<char>` to refer to character sequences|a more general way to present arguments to a function]]:
...
**See also**: [[#Discussion: Use a factory function if you need "virtual behavior" during initialization|Discussion]]
...
**See also**: If the "repeated action" is a simple initialization, consider [[#C.48: Prefer default member initializers to member initializers in constructors for constant initializers|a default member initializer]].
...
It is simple and efficient. If you want to optimize for rvalues, provide an overload that takes an `&&` (see [[#F.18: For "will-move-from" parameters, pass by `X&&` and `std::move` the parameter|F.18]]).
...
By writing directly to the target elements, we will get only [[#the basic guarantee]] rather than the strong guarantee offered by the `swap` technique. Beware of [[#C.62: Make copy assignment safe for self-assignment|self-assignment]].
...
See [[#C.67: A polymorphic class should suppress public copy/move|copy constructor vs. `clone()`]].
...
You can handle self-assignment by explicitly testing for self-assignment, but often it is faster and more elegant to cope without such a test (e.g., [[#C.83: For value-like types, consider providing a `noexcept` swap function|using `swap`]]).
...
**See**: [[#C.60: Make copy assignment non-`virtual`, take the parameter by `const&`, and return by non-`const&`|The rule for copy-assignment]].

##### Enforcement

Equivalent to what is done for [[#C.60: Make copy assignment non-`virtual`, take the parameter by `const&`, and return by non-`const&`|copy-assignment]].
...
The one-in-a-million argument against `if (this == &a) return *this;` tests from the discussion of [[#C.62: Make copy assignment safe for self-assignment|self-assignment]] is even more relevant for self-move.
...
If you need to create deep copies of polymorphic objects, use `clone()` functions: see [[#C.130: For making deep copies of polymorphic classes prefer a virtual `clone` function instead of public copy construction/assignment|C.130]].
...
**See also** [[#C.50: Use a factory function if you need "virtual behavior" during initialization|factory functions]] for how to achieve the effect of a call to a derived class function without risking undefined behavior.
...
A `swap` can be handy for implementing a number of idioms, from smoothly moving objects around to implementing assignment easily to providing a guaranteed commit function that enables strongly error-safe calling code. Consider using swap to implement copy assignment in terms of copy construction. See also [[#E.16: Destructors, deallocation, `swap`, and exception type copy/move construction must never fail|destructors, deallocation, and swap must never fail]].
...
##### Reason
 ...
 [[#C.84: A `swap` function must not fail|A `swap` must not fail]].
...
The standard C++ mechanism to construct an instance of a type is to call its constructor. As specified in guideline [[#C.41: A constructor should create a fully initialized object|C.41]]: a constructor should create a fully initialized object. No additional initialization, such as by `memcpy`, should be required.
...
**See also**: [[#R: Resource management|Resources]]
...
Function objects should be cheap to copy (and therefore [[#F.16: For "in" parameters, pass cheaply-copied types by value and others by reference to `const`|passed by value]]).
...
A pure interface class is simply a set of pure virtual functions; see [[#I.25: Prefer empty abstract classes as interfaces to class hierarchies|I.25]].
...
Another (related) technique for separating interface and implementation is [[#I.27: For stable library ABI, consider the Pimpl idiom|Pimpl]].
...
Generally, it is recommended to use smart pointers to represent ownership (see [[#R.20: Use `unique_ptr` or `shared_ptr` to represent ownership|R.20]]). However, because of language rules, the covariant return type cannot be a smart pointer: `D::clone` can't return a `unique_ptr<D>` while `B::clone` returns `unique_ptr<B>`. Therefore, you either need to consistently return `unique_ptr<B>` in all overrides, or use `owner<>` utility from the [[#GSL.view: Views|Guidelines Support Library]].
...
[[#C.9: Minimize exposure of members|Prefer `private` data]] with a well-specified and enforced invariant.
Alternatively, and often better, [[#C.121: If a base class is used as an interface, make it a pure abstract class|keep data out of any class used as an interface]].
...
* *All public*: If you're writing an aggregate bundle-of-variables without an invariant across those variables, then all the variables should be `public`.
  [[#C.2: Use `class` if the class has an invariant; use `struct` if the data members can vary independently|By convention, declare such classes `struct` rather than `class`]]
...
`Interface` is the root of an [[#C.121: If a base class is used as an interface, make it a pure abstract class|interface hierarchy]]
and `Utility` is the root of an [[#C.129: When designing a class hierarchy, distinguish between implementation inheritance and interface inheritance|implementation hierarchy]].
...
[[#C.67: A polymorphic class should suppress public copy/move|A polymorphic class should suppress copying]]
...
[[#C.153: Prefer virtual function to casting|Prefer virtual functions to casting]].
...
Contrast with [[#C.147: Use `dynamic_cast` to a reference type when failure to find the required class is considered an error|C.147]], where failure is an error, and should not be used for conditional execution.
...
### C.150: Use `make_unique()` to construct objects owned by `unique_ptr`s
See [[#R.23: Use `make_unique()` to make `unique_ptr`s|R.23]]

### C.151: Use `make_shared()` to construct objects owned by `shared_ptr`s
See [[#R.22: Use `make_shared()` to make `shared_ptr`s|R.22]]
...
See [[#C.146: Use `dynamic_cast` where class hierarchy navigation is unavoidable|C.146]] and ???
...
Here, the conventional semantics is maintained: [[#C.copy: Copy and move|Copies compare equal]].
...
Non-member operators should be either friends or defined in [[#C.168: Define overloaded operators in the namespace of their operands|the same namespace as their operands]].
[[#C.161: Use non-member functions for symmetric operators|Binary operators should treat their operands equivalently]].
...
This is a special case of the rule that [[#C.5: Place helper functions in the same namespace as the class they support|helper functions should be defined in the same namespace as their class]].
...
* [[#C.180: Use `union`s to save memory|C.180: Use `union`s to save Memory]]
...
But heed the warning: [[#C.181: Avoid "naked" `union`s|Avoid "naked" `union`s]]
...
* [[#Enum.3: Prefer class enums over "plain" enums|Enum.3: Prefer `enum class`es over "plain" `enum`s]]
...
Where a resource is "ill-behaved" in that it isn't represented as a class with a destructor, wrap it in a class or use [[#E.19: Use a `final_action` object to express cleanup if no suitable resource handle is available|`finally`]]

**See also**: [[#E.6: Use RAII to prevent leaks|RAII]]
...
In addition to suffering from the problem of [[#P.8: Don't leak any resources|leak]], this adds a spurious allocation and deallocation operation, and is needlessly verbose. If Gadget is cheap to move out of a function (i.e., is small or has an efficient move operation), just return it "by value" (see [[#F.20: For "out" output values, prefer return values to output parameters|"out" return values]]):
...
**See also**: [[#R.3: A raw pointer (a `T*`) is non-owning|The raw pointer rule]]

##### Enforcement

See [[#R.3: A raw pointer (a `T*`) is non-owning|the raw pointer rule]]
...
### R.6: Avoid non-`const` global variables
See [[#I.2: Avoid non-`const` global variables|I.2]]
...
### R.30: Take smart pointers as parameters only to explicitly express lifetime semantics
See [[#F.7: For general use, take `T*` or `T&` arguments rather than smart pointers|F.7]].
...
Both cases are an error under the [[#R.30: Take smart pointers as parameters only to explicitly express lifetime semantics|`sharedptrparam` guideline]]:
...
For naming, commenting, and indentation rules, see [[#NL: Naming and layout suggestions|NL: Naming and layout]].
...
* [[#ES.46: Avoid lossy (narrowing, truncating) arithmetic conversions|ES.46: Avoid narrowing conversions]]
...
**See also**: [[#SL.1:  Use libraries wherever possible|SL.1]], [[#ES.11: Use `auto` to avoid redundant repetition of type names|ES.11]]
...
**See also**: [[#ES.26: Don't use a variable for two unrelated purposes|Don't use a variable for two unrelated purposes]]
...
we can eliminate it by using a lambda [[#ES.28: Use lambdas for complex initialization, especially of `const` variables|ES.28]]:
...
**See also**: [[#ES.28: Use lambdas for complex initialization, especially of `const` variables|ES.28]]
...
For initializers of moderate complexity, including for `const` variables, consider using a lambda to express the initializer; see [[#ES.28: Use lambdas for complex initialization, especially of `const` variables|ES.28]].
...
Avoid macros if you can: [[#ES.30: Don't use macros for program text manipulation|ES.30]], [[#ES.31: Don't use macros for constants or "functions"|ES.31]], and [[#ES.32: Use `ALL_CAPS` for all macro names|ES.32]].
...
(left-to-right except right-to-left in assignments, and the order of evaluation of function arguments is unspecified; [[#ES.43: Avoid expressions with undefined order of evaluation|see ES.43]]),
...
Pointers should [[#I.13: Do not pass an array as a single pointer|only refer to single objects]].
...
This rule is part of the [[#Pro.bounds: Bounds safety profile|bounds-safety profile]].
...
* Flag functional style casts using `Type(value)`. Use `Type{value}` instead which is not narrowing. (See [[#ES.64: Use the `T{e}`notation for construction|ES.64]].)
...
A move typically leaves behind an empty object ([[#C.64: A move operation should move and leave its source in a valid state|C.64]]), which can be surprising or even dangerous, so we try to avoid moving from lvalues (they might be accessed later).
...
And after you do that, assume the object has been moved from (see [[#C.64: A move operation should move and leave its source in a valid state|C.64]]) and don't read its state again until you first set it to a new value.
...
This example not only violates the [[#ES.60: Avoid `new` and `delete` outside resource management functions|no naked `new` rule]] as in the previous example, it has many more problems.
...
The first defense against this is to [[#C.67: A polymorphic class should suppress public copy/move|define the base class `Shape` not to allow this]].
...
[[#ES.48: Avoid casts|Avoid casts]] (explicit type conversion) and if you must [[#ES.49: If you must use a cast, use a named cast|prefer named casts]].
...
The construction notation is the most general [[#ES.23: Prefer the `{}`-initializer syntax|initializer notation]].
...
* Use [[#R.1: Manage resources automatically using resource handles and RAII (Resource Acquisition Is Initialization)|RAII]] to avoid lifetime problems.
* Use [[#F.26: Use a `unique_ptr<T>` to transfer ownership where a pointer is needed|unique_ptr]] to avoid lifetime problems.
* Use [[#F.27: Use a `shared_ptr<T>` to share ownership|shared_ptr]] to avoid lifetime problems.
* Use [[#F.60: Prefer `T*` over `T&` when "no argument" is a valid option|references]] when `nullptr` isn't a possibility.
* Use [[#F.23: Use a `not_null<T>` to indicate that "null" is not a valid value|not_null]] to catch unexpected `nullptr` early.
* Use the [[#Pro.bounds: Bounds safety profile|bounds profile]] to avoid range errors.
...
This rule is part of the [[#Pro.lifetime: Lifetime safety profile|lifetime safety profile]]
...
### ES.74: Prefer to declare a loop variable in the initializer part of a `for`-statement
See [[#ES.6: Declare names in for-statement initializers and conditions to limit scope|ES.6]]
...
If you really need to break out a loop, a `break` is typically better than alternatives such as [[#ES.86: Avoid modifying loop control variables inside the body of raw for-loops|modifying the loop variable]] or a [[#ES.76: Avoid `goto`|`goto`]]:
...
This precludes consistency. Use `gsl::index` for subscripts; [[#ES.107: Don't use `unsigned` for subscripts, prefer `gsl::index`|see ES.107]].
...
This makes surprises (and bugs) inevitable.
 ...
 Use `gsl::index` for subscripts; [[#ES.107: Don't use `unsigned` for subscripts, prefer `gsl::index`|see ES.107]].
...
Consider:
    ...
    unsigned area(unsigned height, unsigned width) { return height*width; } // [[#I.6: Prefer `Expects()` for expressing preconditions|see also]]
    // ...
    int height;
    cin >> height;
...
We can do better (in C++98)
    ...
    template<typename Iter>
        void sort(Iter b, Iter e);  // sort [[#Per.2: Don't optimize prematurely|b:e)
...
* Information passing:
Prefer clean [[#I: Interfaces|interfaces]] carrying sufficient information for later improvement of implementation.
Note that information flows into and out of an implementation through the interfaces we provide.
* Compact data: By default, [[#Per.16: Use compact data structures|use compact data]], such as `std::vector` and [[#Per.19: Access memory predictably|access it in a systematic fashion]].
...
Use [[#F.15: Prefer simple and conventional ways of passing information|conventional ways]] of passing information through an interface;
...
A need for efficiency does not imply a need for [[#Per.5: Don't assume that low-level code is necessarily faster than high-level code|low-level code]].
...
This rule does not contradict the [[#Per.2: Don't optimize prematurely|Don't optimize prematurely]] rule.
...
For further information of how to use synchronization well to eliminate data races, please consult a good book about concurrency (see [[#CP.102: Carefully study the literature|Carefully study the literature]]).
...
See also [[#CP.mess: Message passing|CP.mess: Message Passing]] and [[#CP.31: Pass small amounts of data between threads by value, rather than by reference or pointer|CP.31: prefer pass by value]].
...
[[#CP.200: Use `volatile` only to talk to non-C++ memory|(rare) proper uses of `volatile`]]
...
* For parallel algorithms, see [[#CP.par: Parallelism|parallelism]]
* For inter-task communication without explicit sharing, see [[#CP.mess: Message passing|messaging]]
* For vector parallel code, see [[#CP.vec: Vectorization|vectorization]]
* For lock-free programming, see [[#CP.free: Lock-free programming|lock free]]
...
This rule is redundant if you [[#CP.26: Don't `detach()` a thread|don't `detach()`]] and [[#CP.25: Prefer `gsl::joining_thread` over `std::thread`|use `gsl::joining_thread`]].
...
[[#CP.26: Don't `detach()` a thread|Don't `detach`]].
...
* Suggest [[#CP.26: Don't `detach()` a thread|"exporting ownership"]] to an enclosing scope if it detaches.
...
But that compromises safety and violates the [[#CP.20: Use RAII, never plain `lock()`/`unlock()`|use RAII]] rule.
...
Similar to [[#R.12: Immediately give the result of an explicit resource allocation to a manager object|R.12]], which tells you to avoid raw owning pointers, you should
...
In C++, unlike in some other languages, `volatile` has [[#CP.8: Don't try to use `volatile` for synchronization|nothing to do with synchronization]].
...
* A constructor that cannot construct an object (failure to establish its class's [[#C.2: Use `class` if the class has an invariant; use `struct` if the data members can vary independently|invariant]])
...
**See also**: [[#Discussion: Usage of noexcept|discussion]]

##### Note

Before deciding that you cannot afford or don't like exception-based error handling, have a look at the [[#E.25: If you can't throw exceptions, simulate RAII for resource management|alternatives]];
...
An [[#C.2: Use `class` if the class has an invariant; use `struct` if the data members can vary independently|invariant]] is a logical condition for the members of an object that a constructor must establish for the public member functions to assume.
...
**See also**: [[#C.42: If a constructor cannot construct a valid object, throw an exception|If a constructor cannot construct a valid object, throw an exception]]
...
as a last resort, cleanup actions can be represented by a [[#E.19: Use a `final_action` object to express cleanup if no suitable resource handle is available|`final_action`]] object.
...
**See also**: [[#E.12: Use `noexcept` when exiting a function because of a `throw` is impossible or unacceptable|Use of `noexcept`]]
...
**See also**: [[#I.5: State preconditions (if any)|precondition rule]]
...
**See also**: [[#I.7: State postconditions|postcondition rule]]
...
Do not use traditional [[#E.30: Don't use exception specifications|exception-specifications]].

##### See also

[[#Discussion: Usage of noexcept|discussion]].
...
Sometimes, [[#E.19: Use a `final_action` object to express cleanup if no suitable resource handle is available|`finally()`]] can make such unsystematic cleanup a bit more manageable.
...
Most handlers do not modify their exception and in general we [[#ES.25: Declare an object `const` or `constexpr` unless you want to modify its value later on|recommend use of `const`]].
...
* Also, [[#C.66: Make move operations `noexcept`|make move operations `noexcept`]].
...
**See also**: [[#Discussion: Destructors, deallocation, and swap must never fail|discussion]]
...
Let cleanup actions on the unwinding path be handled by [[#E.6: Use RAII to prevent leaks|RAII]].
...
* proper resource handles and [[#E.6: Use RAII to prevent leaks|RAII]]
* [[#E.19: Use a `final_action` object to express cleanup if no suitable resource handle is available|`finally`]]
...
Prefer [[#E.6: Use RAII to prevent leaks|proper resource management objects]].
Consider `finally` a last resort.

##### Note

Use of `finally` is a systematic and reasonably clean alternative to the old [[#E.27: If you can't throw exceptions, use error codes systematically|`goto exit;` technique]]
...
Even without exceptions, [[#E.6: Use RAII to prevent leaks|RAII]] is usually the best and most systematic way of dealing with resources.
...
So, the primary guideline for error handling is "use exceptions and [[#E.6: Use RAII to prevent leaks|RAII]]."
...
Before condemning exceptions or complaining too much about their cost, consider examples of the use of [[#E.27: If you can't throw exceptions, use error codes systematically|error codes]].
...
**See also**: [[#E.25: If you can't throw exceptions, simulate RAII for resource management|Simulating RAII]]
...
This approach fits with [[#E.25: If you can't throw exceptions, simulate RAII for resource management|simulated RAII resource management]].
...
`finally` can [[#E.19: Use a `final_action` object to express cleanup if no suitable resource handle is available|ease the pain a bit]].
...
We [[#E.2: Throw an exception to signal that a function can't perform its assigned task|prefer exception-based error handling]] and recommend [[#F.3: Keep functions short and simple|keeping functions short]].

**See also**: [[#Discussion]]

**See also**: [[#F.21: To return multiple "out" values, prefer returning a struct|Returning multiple values]]
...
If no exception can be thrown, use [[#E.12: Use `noexcept` when exiting a function because of a `throw` is impossible or unacceptable|`noexcept`]].
...
* "cast away `const`"; [[#ES.50: Don't cast away `const`|best avoided]]
...
**See also**: [[#I.27: For stable library ABI, consider the Pimpl idiom|Pimpl]]
...
[[#ES.50: Don't cast away `const`|Do not cast away `const`]].
...
* [[#T.23: Differentiate a refined concept from its more general case by adding new use patterns.|T.23: Differentiate a refined concept from its more general case by adding new use patterns]]
* [[#T.24: Use tag classes or traits to differentiate concepts that differ only in semantics.|T.24: Use tag classes or traits to differentiate concepts that differ only in semantics]]
...
* [[#T.65: Use tag dispatch to provide alternative implementations of a function|T.65: Use tag dispatch to provide alternative implementations of functions]]
...
* [[#T.102: How to process arguments to a variadic template|T.102: ??? How to process arguments to a variadic template ???]]
...
* [[#T.142?: Use template variables to simplify notation|T.142: Use template variables to simplify notation]]
...
See [[#T.84: Use a non-template core implementation to provide an ABI-stable interface|Stable base]].
...
Examples include type erasure as with `std::shared_ptr`'s deleter (but [[#T.49: Where possible, avoid type-erasure|don't overuse type erasure]]).
...
* [[#T.10: Specify concepts for all template arguments]]
* [[#T.11: Whenever possible use standard concepts]]
* [[#T.12: Prefer concept names over `auto` for local variables|T.12: Prefer concept names over `auto`]]
* [[#T.13: Prefer the shorthand notation for simple, single-type argument concepts]]
* ???
...
* [[#T.23: Differentiate a refined concept from its more general case by adding new use patterns.|T.23: Differentiate a refined concept from its more general case by adding new use patterns]]
* [[#T.24: Use tag classes or traits to differentiate concepts that differ only in semantics.|T.24: Use tag classes or traits to differentiate concepts that differ only in semantics]]
* [[#T.25: Avoid complementary constraints|T.25: Avoid complimentary constraints]]
* [[#T.26: Prefer to define concepts in terms of use-patterns rather than simple syntax]]
* ???
...
##### Reason
 ...
 "Standard" concepts (as provided by the [[#GSL]] and the ISO standard itself)
...
This is a specific variant of the general rule that [[#T.20: Avoid "concepts" without meaningful semantics|a concept must make semantic sense]].
...
See the [[#RF: References|Palo Alto TR]] for examples.
...
To disambiguate them, see [[#T.24: Use tag classes or traits to differentiate concepts that differ only in semantics.|T.24]].
...
Beware of [[#T.25: Avoid complementary constraints|complementary constraints]].
...
**See also**: [[#T.69: Inside a template, don't make an unqualified non-member function call unless you intend it to be a customization point|T.69]]
...
For N == 1, we have a choice of a base class of a class in the surrounding scope as in [[#T.61: Do not over-parameterize members (SCARY)|T.61]].
...
this can cause problems like [[#T.47: Avoid highly visible unconstrained templates with common names|unintentionally invoking unconstrained function templates]].
...
see [[#T.84: Use a non-template core implementation to provide an ABI-stable interface|"stable base"]] and [[#T.5: Combine generic and OO techniques to amplify their strengths, not their costs|OO and GP]]
...
Note that `maul()` violates the a [[#F.22: Use `T*` or `owner<T*>` to designate a single object|`T*` points to an individual object rule]].
...
Note that the assignment in `maul2()` violated the [[#ES.63: Don't slice|no-slicing rule]].
...
**Alternative**: Use a [[#I.27: For stable library ABI, consider the Pimpl idiom|Pimpl]] implementation.
...
Instead, use concepts. But see [[#T.121: Use template metaprogramming primarily to emulate concepts|How to emulate concepts if you don't have language support]].

##### Example

    ??? good

**Alternative**: If the result is a value, rather than a type, use a [[#T.123: Use `constexpr` functions to compute values at compile time|`constexpr` function]].
...
### T.140: If an operation can be reused, give it a name
See [[#F.10: If an operation can be reused, give it a name|F.10]]

### T.141: Use an unnamed lambda if you need a simple function object in one place only
See [[#F.11: Use an unnamed lambda if you need a simple function object in one place only|F.11]]
...
### SF.1: Use a `.cpp` suffix for code files and `.h` for interface files if your project doesn't already follow another convention
See [[#NL.27: Use a `.cpp` suffix for code files and `.h` for interface files|NL.27]]
...
[[#SF.7: Don't write `using namespace` at global scope in a header file|Don't write `using namespace` at global scope in a header file]].
...
Our recommendation is to write in ISO C++: See [[#P.2: Write in ISO Standard C++|rule P.2]].
...
* [[#SL.1:  Use libraries wherever possible|SL.1: Use libraries wherever possible]]
...
[[#ES.23: Prefer the `{}`-initializer syntax|Prefer the {}-initializer syntax]].
...
This rule is part of the [[#Pro.bounds: Bounds safety profile|bounds profile]].
...
**See also**: [[#SL.regex: Regex|regular expressions]]
...
* [[#F.24: Use a `span<T>` or a `span_p<T>` to designate a half-open sequence|F.24 span]]
* [[#F.25: Use a `zstring` or a `not_null<zstring>` to designate a C-style string|F.25 zstring]]
...
[[#Per.2: Don't optimize prematurely|Don't optimize prematurely]]
...
See [[#SL.str.3: Use `zstring` or `czstring` to refer to a C-style, zero-terminated, sequence of characters|`zstring`]], [[#SL.str.1: Use `std::string` to own character sequences|`string`]], and [[#SL.str.2: Use `std::string_view` or `gsl::span<char>` to refer to character sequences|`string_view`]].
...
`std::string` supports standard-library [[#SL.str.10: Use `std::string` when you need to perform locale-sensitive string operations|`locale` facilities]]
...
* [[#SL.io.3: Prefer `iostream`s for I/O|SL.io.3: Prefer iostreams for I/O]]
...
* [[#ES.20: Always initialize an object|Always initialize an object]]
* [[#ES.21: Don't introduce a variable (or constant) before you need to use it]]
...
Also, this style is a temptation to use the [[#NR.6: Don't place all cleanup actions at the end of a function and `goto exit`|goto exit]] non-rule.
...
* [[#E.6: Use RAII to prevent leaks|RAII]]
...
* Use exceptions and [[#E.6: Use RAII to prevent leaks|RAII]]
* for non-RAII resources, use [[#E.19: Use a `final_action` object to express cleanup if no suitable resource handle is available|`finally`]].
...
* [[#RF.C++: C++ Programming (C++11/C++14)|RF.C++: C++ Programming (C++11/C++14/C++17)]]
* [[#RF.web: Websites]]
...
* [[#Stroustrup05]] Bjarne Stroustrup: [A rationale for semantically enhanced library languages](http://www.stroustrup.com/SELLrationale.pdf).
  LCSD05. October 2005.
* [[#Stroustrup14]] Stroustrup: [A Tour of C++](http://www.stroustrup.com/Tour.html).
  Addison-Wesley 2014.
  Each chapter ends with an advice section consisting of a set of recommendations.
* [[#Stroustrup13]] Stroustrup: [The C++ Programming Language (4th Edition)](http://www.stroustrup.com/4th.html).
  Addison-Wesley 2013.
  Each chapter ends with an advice section consisting of a set of recommendations.
...
We must aim for [[#Appendix B: Modernizing code|gradual adoption]].
...
* [[#Pro.safety: Type-safety profile|Pro.type: Type safety]]
* [[#Pro.bounds: Bounds safety profile|Pro.bounds: Bounds safety]]
* [[#Pro.lifetime: Lifetime safety profile|Pro.lifetime: Lifetime safety]]
...
Note that the safety is intended to be complete when combined also with [[#Pro.bounds: Bounds safety profile|Bounds safety]] and [[#Pro.lifetime: Lifetime safety profile|Lifetime safety]].

An implementation of this profile shall recognize the following patterns in source code as non-conforming and issue a diagnostic.

Type safety profile summary:

* Type.1: [[#ES.48: Avoid casts|Avoid casts]]:

  1. Don't use `reinterpret_cast`; A strict version of [[#ES.48: Avoid casts|Avoid casts]] and [[#ES.49: If you must use a cast, use a named cast|prefer named casts]].
  2. Don't use `static_cast` for arithmetic types; A strict version of [[#ES.48: Avoid casts|Avoid casts]] and [[#ES.49: If you must use a cast, use a named cast|prefer named casts]].
  3. Don't cast between pointer types where the source type and the target type are the same; A strict version of [[#ES.48: Avoid casts|Avoid casts]].
  4. Don't cast between pointer types when the conversion could be implicit; A strict version of [[#ES.48: Avoid casts|Avoid casts]].
* Type.2: Don't use `static_cast` to downcast:
[[#C.146: Use `dynamic_cast` where class hierarchy navigation is unavoidable|Use `dynamic_cast` instead]].
* Type.3: Don't use `const_cast` to cast away `const` (i.e., at all):
[[#ES.50: Don't cast away `const`|Don't cast away const]].
* Type.4: Don't use C-style `(T)expression` or functional `T(expression)` casts:
Prefer [[#ES.64: Use the `T{e}`notation for construction|construction]] or [[#ES.49: If you must use a cast, use a named cast|named casts]] or `T{expression}`.
* Type.5: Don't use a variable before it has been initialized:
[[#ES.20: Always initialize an object|always initialize]].
* Type.6: Always initialize a data member:
[[#ES.20: Always initialize an object|always initialize]],
possibly using [[#C.43: Ensure that a copyable class has a default constructor|default constructors]] or
[[#C.48: Prefer default member initializers to member initializers in constructors for constant initializers|default member initializers]].
* Type.7: Avoid naked union:
[[#C.181: Avoid "naked" `union`s|Use `variant` instead]].
* Type.8: Avoid varargs:
[[#F.55: Don't use `va_arg` arguments|Don't use `va_arg` arguments]].
...
Note that this type-safety can be complete only if we also have [[#Pro.bounds: Bounds safety profile|Bounds safety]] and [[#Pro.lifetime: Lifetime safety profile|Lifetime safety]].
...
Bounds safety is intended to be complete only when combined with [[#Pro.safety: Type-safety profile|Type safety]] and [[#Pro.lifetime: Lifetime safety profile|Lifetime safety]],
...
[[#I.13: Do not pass an array as a single pointer|Pass pointers to single objects (only)]] and [[#ES.42: Keep use of pointers simple and straightforward|Keep pointer arithmetic simple]].
...
[[#SL.con.3: Avoid bounds errors|Use the standard library in a type-safe manner]].
...
[[#ES.65: Don't dereference an invalid pointer|detect or avoid]].
...
References are never owners (see [[#R.4: A raw reference (a `T&`) is non-owning|R.4]]). Note: References have many opportunities to outlive the objects they refer to (returning a local variable by reference, holding a reference to an element of a vector and doing `push_back`, binding to `std::max(x, y + 1)`, etc). The Lifetime safety profile aims to address those things, but even so `owner<T&>` does not make sense and is discouraged.
...
Often, you don't have a choice and must follow an established style for [[#NL.8: Use a consistent naming style|consistency]].
The need for consistency beats personal taste.

This is a recommendation for [[#NL: Naming and layout suggestions|when you have no constraints or better ideas]].
...
Literals should not be sprinkled all over the code as [[#ES.45: Avoid "magic constants"; use symbolic constants|"magic constants"]],
...
For details, see [[#ES.10: Declare one name (only) per declaration|ES.10]].
...
* We could convert a code base module for module, but any rules that affects interfaces (especially ABIs), such as [[#GSL.view: Views|use `span`]], cannot be done on a per-module basis.
...
* [[#Discussion: Provide strong resource safety; that is, never leak anything that you think of as a resource|Provide strong resource safety; that is, never leak anything that you think of as a resource]]
* [[#Discussion: Never return or throw while holding a resource not owned by a handle|Never return or throw while holding a resource not owned by a handle]]
* [[#Discussion: A "raw" pointer or reference is never a resource handle|A "raw" pointer or reference is never a resource handle]]
* [[#Discussion: Never let a pointer outlive the object it points to|Never let a pointer outlive the object it points to]]
* [[#Discussion: Use templates to express containers (and other resource handles)|Use templates to express containers (and other resource handles)]]
* [[#Discussion: Return containers by value (relying on move or copy elision for efficiency)|Return containers by value (relying on move or copy elision for efficiency)]]
* [[#Discussion: If a class is a resource handle, it needs a constructor, a destructor, and copy and/or move operations|If a class is a resource handle, it needs a constructor, a destructor, and copy and/or move operations]]
* [[#Discussion: If a class is a container, give it an initializer-list constructor|If a class is a container, give it an initializer-list constructor]]
...
This class is a resource handle. It manages the lifetime of the `T`s. To do so, `Vector` must define or delete [[#C.21: If you define or `=delete` any copy, move, or destructor function, define or `=delete` them all|the copy, move, and destruction operations]].
...
See the Exceptions in [[#F.20: For "out" output values, prefer return values to output parameters|F.20]].
...
In general, a tool cannot know if a class is a resource handle. However, if a class has some of [[#C.ctor: Constructors|the default operations]], it should have all, and if a class has a member that is a resource handle, it should be considered as resource handle.
